...
# =======================
# Parallel shards runner
# =======================
from concurrent.futures import ProcessPoolExecutor, as_completed
import traceback

def _process_one_shard(args):
    """Child process entry. args is a dict (picklable)."""
    shard_idx: int = args["shard_idx"]
    shard_path: Path = Path(args["shard_path"])
    out_root: Path = Path(args["out_root"])
    shard_name_fmt: str = args["shard_name_fmt"]
    # Optional knobs
    subset_size = args.get("subset_size", None)
    max_filesize_mb = args.get("max_filesize_mb", None)
    resume = args.get("resume", True)
    use_dual_base = args.get("use_dual_base", True)

    try:
        shard_name = shard_name_fmt.format(idx=shard_idx)
        jsonl_out  = out_root / f"{shard_name}.jsonl"
        images_dir = out_root / "images" / "train" / f"shard-{shard_idx:05d}"

        build_pubmed_image_caption_dataset_streaming(
            file_list_path=shard_path,
            jsonl_out_path=jsonl_out,
            images_out_dir=images_dir,
            subset_size=subset_size,
            max_filesize_mb=max_filesize_mb,
            resume=resume,
            use_dual_base=use_dual_base,
        )
        return {"shard_idx": shard_idx, "shard_path": str(shard_path), "ok": True, "msg": "done"}
    except Exception as e:
        return {
            "shard_idx": shard_idx,
...
def run_train_shards_parallel(
    shard_dir: Path,
    out_root: Path = repo_root / "_results/data/pmc15_cc12m_like",
    num_workers: int = 4,
    shard_name_fmt: str = "train_shard-{idx:05d}",
    ...
    """
    Parallel version:
      - Finds all *.txt shard lists under shard_dir
      - Launches up to num_workers processes
      - Each process produces its own JSONL + images folder
    """
    shard_lists = sorted([p for p in Path(shard_dir).glob("*.txt")])
    if not shard_lists:
        print(f"[parallel] No shard list files in {shard_dir}")
        return

    print(f"[parallel] Found {len(shard_lists)} shards in {shard_dir}")
    print(f"[parallel] Using {num_workers} workers")

    tasks = []
    for i, shard_path in enumerate(shard_lists, start=1):
        tasks.append({
            "shard_idx": i,
            "shard_path": str(shard_path),
            "out_root": str(out_root),
            "shard_name_fmt": shard_name_fmt,
            "subset_size": subset_size,
            "max_filesize_mb": max_filesize_mb,
            "resume": resume,
            "use_dual_base": use_dual_base,
    ...
    with ProcessPoolExecutor(max_workers=num_workers) as ex:
        future_map = {ex.submit(_process_one_shard, t): t for t in tasks}
        for fut in as_completed(future_map):
            res = fut.result()
            prefix = f"[shard {res['shard_idx']:05d}]"
            if res["ok"]:
                ok_cnt += 1
                print(f"{prefix} OK: {res['shard_path']}")
            else:
                fail_cnt += 1
                print(f"{prefix} FAIL: {res['shard_path']}\n{res['msg']}")

    print(f"[parallel] Done. OK={ok_cnt}, FAIL={fail_cnt}, TOTAL={len(tasks)}")
...
if __name__ == "__main__":
    ...
    # New: parallel options (shards only)
    parser.add_argument("--num_workers", type=int, default=4)
    parser.add_argument("--subset_size", type=int, default=None)
    parser.add_argument("--max_filesize_mb", type=int, default=None)
    parser.add_argument("--no_resume", action="store_true", help="Disable resume (rewrite JSONL lines if repeated)")
